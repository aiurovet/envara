...
class TestEnvFileLoad:
    ...
    def test_load_method_gets_files(self, mocker: MockerFixture):
        mock_get_files = mocker.patch.object(EnvFile, "get_files", return_value=[])
    ...
    def test_load_method_reads_text(self, mocker: MockerFixture):
        mocker.patch.object(EnvFile, "get_files", return_value=[])
        mock_read_text = mocker.patch.object(EnvFile, "read_text", return_value="")
        mocker.patch.object(EnvFile, "load_from_str")
    ...
    def test_load_from_str_deletes_existing_key(self, monkeypatch: MonkeyPatch):
        monkeypatch.setenv("EXISTING_KEY", "original")
        EnvFile.load_from_str("EXISTING_KEY=")
        assert "EXISTING_KEY" not in os.environ

    def test_load_from_str_deletes_key_from_environ(self, monkeypatch: MonkeyPatch):
    ...
    def test_load_from_str_expands_value(self, monkeypatch: MonkeyPatch):
    ...
    def test_load_from_str_trims_key(self):
    ...
    def test_load_from_str_with_custom_args(self):
        EnvFile.load_from_str("KEY=$1", args=["arg1"])
        assert os.environ.get("KEY") == "arg1"

    def test_load_from_str_with_custom_expand_flags(self):
...
class TestEnvFileReadText:
    ...
    def test_read_text_returns_string(self, mocker: MockerFixture):
        mock_path = MagicMock()
    ...
    def test_read_text_accumulates_files(self, mocker: MockerFixture):
        ...
        mock_file2.configure_mock(**{"__str__.return_value": "/path/file2"})
