...
def test_read_text_skip_defaults(mocker):
    # Arrange
    DotEnv._loaded = []
    ...
    # Act
    content = DotEnv.read_text(m_path, file_flags=DotEnvFileFlags.SKIP_DEFAULT_FILES)
...
def test_read_text_visible_files_flag(mocker):
    # Arrange
    DotEnv._loaded = []
    m_get_stack = mocker.patch("dotenv.Env.get_platform_stack", return_value=["env"])
    m_cwd = mocker.MagicMock(spec=Path)
    m_cwd.is_absolute.return_value = True
    m_cwd.absolute.return_value = m_cwd
    mocker.patch.object(Path, "cwd", return_value=m_cwd)
